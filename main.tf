...
resource "aws_subnet" "more_priv_subnet" {
  vpc_id            = aws_vpc.more_vpc.id
...
resource "aws_route_table" "more_pub_rt" {
  ...
  route {
    ...
    gateway_id = aws_internet_gateway.more_igw.id
  }
...
resource "aws_lb_listener" "more_listener" {
  load_balancer_arn = aws_lb.more_alb.arn
  port              = "80"
  protocol          = "HTTP"
...
resource "aws_ecs_service" "more_ecs_service" {
  ...
  network_configuration {
    subnets         = [aws_subnet.more_priv_subnet.id]
    security_groups = [aws_security_group.more_ecs_sg.id]
...
###############################################################################
resource "aws_appautoscaling_target" "more_ecs_asg_target" {
  service_namespace  = "ecs"
  resource_id        = "service/${aws_ecs_cluster.more_ecs_cluster.name}/${aws_ecs_service.more_ecs_service.name}"
...
resource "aws_appautoscaling_policy" "more_ecs_asg_scale_out" {
  ...
  service_namespace  = aws_appautoscaling_target.more_ecs_asg_target.service_namespace
